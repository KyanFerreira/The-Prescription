...
describe("The document", () => {
  ...
  test("displays the correct cost for the user's input values after clicking the calculate button", () => {
    ...
    price.value = 20;// Set the price 
    const subscribed = document.getElementById('subscribed');
    subscribed.value = false; // set the subscribed as false 
    const refills = document.getElementById('refills');
    refills.value = 2; // set the refill to 2 to test if it becomes 40
    const calculateButton = getByRole(document, "button", {
      name: /calculate/i,
    });

    calculateButton.click();
const cost = document.getElementById('cost'); // get the cost to see if it fired

    // TODO: Change this assertion to check the correct value.
    expect(cost.value).toBe("$40.00");
  });
});
